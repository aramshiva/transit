...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    const response = await fetch(`https://api.pugetsound.onebusaway.org/api/where/report-problem-with-stop/${id}.json?key=${process.env.ONEBUSAWAY_API_KEY}&userComment=${comment}&code=${code}`);
